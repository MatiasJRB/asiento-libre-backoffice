    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-3xl font-bold text-neutral-900">{profile.full_name || 'Sin nombre'}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div>
                <span className="text-neutral-600">Verificación:</span>{' '}
              ...
              <div>
                ...
                <span className={`px-2 py-1 rounded text-xs ${
                  ...
                  profile.role === 'admin' ? 'bg-primary-light text-primary-dark' :
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div>
                <span className="text-neutral-600">Total calificaciones:</span>{' '}
                <span className="font-medium">{profile.ratings_count}</span>
              </div>
              <div>
                <span className="text-neutral-600">Viajes publicados:</span>{' '}
                <span className="font-medium">{ridesAsDriver.length}</span>
              </div>
              <div>
                <span className="text-neutral-600">Solicitudes de viaje:</span>{' '}
                <span className="font-medium">{ridesAsPassenger.length}</span>
              </div>
              <div>
                <span className="text-neutral-600">Vehículos registrados:</span>{' '}
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              {profile.suspended && (
                <>
                  <div>
                    <span className="text-neutral-600">Fecha de suspensión:</span>{' '}
                    <span className="font-medium">
                      {profile.suspended_at ? new Date(profile.suspended_at).toLocaleDateString('es-AR') : '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-neutral-600">Razón:</span>{' '}
                    <p className="text-neutral-900 mt-1">{profile.suspended_reason || '-'}</p>
                  </div>
                </>
              )}
              <div>
                  ...
                  <div key={rating.id} className="border-b pb-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        {rating.comment && (
                          <p className="text-sm text-neutral-700 mt-1">&ldquo;{rating.comment}&rdquo;</p>
                        )}
                      </div>
